...
const Slider = ({ product }) => {
    ...
    return <div>
        ...
        <div className=" my-10 grid grid-cols-5 gap-6">
            {
                images?.map(image => <button onClick={() => setCurrentImage(image)} className={`w-full h-full ${currentImage === image && "border-2 border-orange-500"} rounded-lg`} key={image}><img className="w-full h-28 rounded-lg" src={image} alt="" /></button>)
            }
        </div>
    </div>
